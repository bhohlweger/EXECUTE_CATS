...
void RUN2_main(const unsigned& NumIter, const unsigned& NumJobs,
	...
	const TString ResMatrixFileName = Form("%s/run2_decay_matrices_old.root",
			CalibBaseDir.Data());
	//      "/Users/bernhardhohlweger/Dropbox/FastForwardFemto/CATS_Files/DecayMatrices/run2_decay_matrices_old.root";
	...
	const TString SigmaMatrixFileName = Form("%s/Sample3_MeV_compact.root",
			CalibBaseDir.Data());
	//          "/Users/bernhardhohlweger/Dropbox/FastForwardFemto/CATS_Files/ALICE_pp_13TeV/ResolutionMatrices/Sample6_MeV_compact.root":
	//          "/Users/bernhardhohlweger/Dropbox/FastForwardFemto/CATS_Files/ALICE_pPb_5TeV/ResolutionMatrices/Sample3_MeV_compact.root";
	...
	TH2F* hRes_pL_pSigma0_MeV = new TH2F("hRes_pL_pSigma0_MeV",
			"hRes_pL_pSigma0_MeV", hRes_pL_pSigma0->GetNbinsX() / Fraction_Res,
			hRes_pL_pSigma0->GetXaxis()->GetBinLowEdge(1) * UnitConv_Res,
			hRes_pL_pSigma0->GetXaxis()->GetBinUpEdge(
					hRes_pL_pSigma0->GetNbinsX() / Fraction_Res) * UnitConv_Res,
			hRes_pL_pSigma0->GetNbinsY() / Fraction_Res,
			hRes_pL_pSigma0->GetYaxis()->GetBinLowEdge(1) * UnitConv_Res,
			hRes_pL_pSigma0->GetXaxis()->GetBinUpEdge(
					hRes_pL_pSigma0->GetNbinsY() / Fraction_Res)
					* UnitConv_Res);
	TH2F* hRes_pL_pXim_MeV = new TH2F("hRes_pL_pXim_MeV", "hRes_pL_pXim_MeV",
			hRes_pL_pXim->GetNbinsX() / Fraction_Res,
			hRes_pL_pXim->GetXaxis()->GetBinLowEdge(1) * UnitConv_Res,
			hRes_pL_pXim->GetXaxis()->GetBinUpEdge(
					hRes_pL_pXim->GetNbinsX() / Fraction_Res) * UnitConv_Res,
			hRes_pL_pXim->GetNbinsY() / Fraction_Res,
			hRes_pL_pXim->GetYaxis()->GetBinLowEdge(1) * UnitConv_Res,
			hRes_pL_pXim->GetXaxis()->GetBinUpEdge(
					hRes_pL_pXim->GetNbinsY() / Fraction_Res) * UnitConv_Res);
	...
	TH2F* hSigma_pL_MeV = new TH2F("hSigma_pL_MeV", "hSigma_pL_MeV",
			hSigma_pL->GetNbinsX() / Fraction_Sig,
			hSigma_pL->GetXaxis()->GetBinLowEdge(1) * UnitConv_Sig,
			hSigma_pL->GetXaxis()->GetBinUpEdge(
					hSigma_pL->GetNbinsX() / Fraction_Sig) * UnitConv_Sig,
			hSigma_pL->GetNbinsY() / Fraction_Sig,
			hSigma_pL->GetYaxis()->GetBinLowEdge(1) * UnitConv_Sig,
			hSigma_pL->GetXaxis()->GetBinUpEdge(
					hSigma_pL->GetNbinsY() / Fraction_Sig) * UnitConv_Sig);
	TH2F* hSigma_LL_MeV =
			hSigma_LL == NULL ?
					NULL :
					new TH2F("hSigma_LL_MeV", "hSigma_LL_MeV",
							hSigma_LL->GetNbinsX() / Fraction_Sig,
							hSigma_LL->GetXaxis()->GetBinLowEdge(1)
									* UnitConv_Sig,
							hSigma_LL->GetXaxis()->GetBinUpEdge(
									hSigma_LL->GetNbinsX() / Fraction_Sig)
									* UnitConv_Sig,
							hSigma_LL->GetNbinsY() / Fraction_Sig,
							hSigma_LL->GetYaxis()->GetBinLowEdge(1)
									* UnitConv_Sig,
							hSigma_LL->GetXaxis()->GetBinUpEdge(
									hSigma_LL->GetNbinsY() / Fraction_Sig)
									* UnitConv_Sig);
	...
	for (int iBinX = 1; iBinX <= hRes_pL_pSigma0->GetNbinsX() / Fraction_Res;
			iBinX++) {
		for (int iBinY = 1;
				iBinY <= hRes_pL_pSigma0->GetNbinsY() / Fraction_Res; iBinY++) {
			hRes_pL_pSigma0_MeV->SetBinContent(iBinX, iBinY,
					hRes_pL_pSigma0->GetBinContent(iBinX, iBinY));
		}
	}
	for (int iBinX = 1; iBinX <= hRes_pL_pXim->GetNbinsX() / Fraction_Res;
			iBinX++) {
		for (int iBinY = 1; iBinY <= hRes_pL_pXim->GetNbinsY() / Fraction_Res;
				iBinY++) {
			hRes_pL_pXim_MeV->SetBinContent(iBinX, iBinY,
					hRes_pL_pXim->GetBinContent(iBinX, iBinY));
		}
	}
	...
	for (int iBinX = 1; iBinX <= hSigma_pL->GetNbinsX() / Fraction_Sig;
			iBinX++) {
		for (int iBinY = 1; iBinY <= hSigma_pL->GetNbinsY() / Fraction_Sig;
				iBinY++) {
			hSigma_pL_MeV->SetBinContent(iBinX, iBinY,
					hSigma_pL->GetBinContent(iBinX, iBinY));
		}
	}
	if (hSigma_LL && hSigma_LL_MeV) {
		for (int iBinX = 1; iBinX <= hSigma_LL->GetNbinsX() / Fraction_Sig;
				iBinX++) {
			for (int iBinY = 1; iBinY <= hSigma_LL->GetNbinsY() / Fraction_Sig;
					iBinY++) {
				hSigma_LL_MeV->SetBinContent(iBinX, iBinY,
						hSigma_LL->GetBinContent(iBinX, iBinY));
			}
		}
	}
	...
	for (unsigned uIter = FirstIter; uIter <= LastIter; uIter++) {
		...
		if (uIter == 0) {
			...
			vFemReg_pXim = 1;
			vBlReg = 1;
			vMod_pL = 2; // 0 = Usmani (Close to NLO) 1 = NLO Lednicky 2 = LO Lednicky (default)
			vMod_pXim = 1;
			vFrac_pp_pL = 1;
			vFrac_pL_pSigma0 = 1;
			vFrac_pL_pXim = 1;
...
//    vMod_pXim = 0;
		...
		double pXimPotParsI0S0[10] =
				{ 0, 0, pXim_HALQCD1, 13, 0, -1, 1, 0, 0, 0 }; //4th argument is the t parameter and can be:
		double pXimPotParsI0S1[10] =
				{ 0, 0, pXim_HALQCD1, 13, 0, -1, 1, 1, 0, 1 };  // 9, 10, 11, 12
		double pXimPotParsI1S0[10] =
				{ 0, 0, pXim_HALQCD1, 13, 1, 1, 1, 0, 0, 0 }; //This is shit. Corresponds to 9-14 t
		double pXimPotParsI1S1[10] =
				{ 0, 0, pXim_HALQCD1, 13, 1, 1, 1, 1, 0, 1 };  // this value 1-6
		CATS AB_pXim;
		...
		AB_pXim1530.SetRedMass(
				(Mass_p * Mass_Xim1530) / (Mass_p + Mass_Xim1530));
...
//		fitter->FixParameter("pp", DLM_Fitter1::p_sor0, 1.383);
	...
	if (hSigma_pL_MeV) {
		delete hSigma_pL_MeV;
		hSigma_pL_MeV = NULL;
	}
	if (hSigma_LL_MeV) {
		delete hSigma_LL_MeV;
		hSigma_LL_MeV = NULL;
	}
	if (hSigma_pXim_MeV) {
		delete hSigma_pXim_MeV;
		hSigma_pXim_MeV = NULL;
	}
	//delete hSigma_pXim_MeV;
	delete ntResult;
	delete OutFile;
