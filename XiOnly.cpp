...
void GetXiForRadius(TString InputDir, TString tmpOutputDir,
		double GaussSourceSize, const int tOut, TFile *file, bool saveCoulomb);
...
void GetXiForRadius(TString InputDir, TString tmpOutputDir,
	...
	double pXimPotParsI0S0[10] = { 0, 0, pXim_HALQCD1, (double)tOut, 0, -1, 1, 0, 0, 0 };	//4th argument is the t parameter and can be:
	double pXimPotParsI0S1[10] = { 0, 0, pXim_HALQCD1, (double)tOut, 0, -1, 1, 1, 0, 1 };	// 9, 10, 11, 12
	double pXimPotParsI1S0[10] = { 0, 0, pXim_HALQCD1, (double)tOut, 1, 1, 1, 0, 0, 0 };//This is shit. Corresponds to 9-14 t
	double pXimPotParsI1S1[10] = { 0, 0, pXim_HALQCD1, (double)tOut, 1, 1, 1, 1, 0, 1 };// this value 1-6
	CATS AB_pXim;
	...
	FitResult_pXim.SetName(
			TString::Format("FitResult_pXim_%.2f_t_%i", GaussSourceSize,tOut));
...
//	fitter->GetFitGraph(0, Coulomb);
	...
	cfast->SaveAs(
			TString::Format("%scfast_r%.2f.png", tmpOutputDir.Data(),
					GaussSourceSize));
	file->cd();
	if (saveCoulomb) FitResult_pXim_COULOMB.Write();
